...
# %% [markdown]
# The green cluster is very different from the rest.
# The other three clusters have varying densities but are similar.
# Red and blue are different from each other but have some overlap with the cluster in between (yellow).
# The blue cluster is very spread out.
...
#
# - Seems to do a decent job of separating the clusters, even though some points ended up in the wrong cluster.
# - Green/purple is separated from the rest.
# - There's three clusters close to each other, two are separate but both have overlap with the cluster in between, a bit less so than the data.
...
# %% [markdown]
# - Seems to do a decent job of separating the clusters.
# - Green/purple is separated from the rest.
# - There's three clusters close to each other, two are separate but both have overlap with the cluster in between, seems more correct than the t-SNE.
# - 
...
# %% [markdown]
# - seems more reliable than the t-SNE and UMAP
# - a lot quicker than UMAP (similar to t-SNE)
